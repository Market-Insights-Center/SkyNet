...
// Simple utility for class merging if 'cn' is not available
...
export const PinContainer = ({
    ...
    className,
    containerClassName,
    // Add onClick support for navigation handling in parent
    onClick
}) => {
        ...
        >
            ...
            >
                <div
                    ...
                    className="absolute left-1/2 p-0 top-1/2 flex justify-start items-start rounded-3xl shadow-[0_8px_16px_rgb(0_0_0/0.4)] bg-transparent transition duration-700 overflow-visible w-full h-full"
                >
                    <div className={cn("relative z-50 w-full h-full", className)}>{children}</div>
                </div>
            </div>
            {/* Provide title only if passed, used for the 'pin' pop-up effect */}
...
export const PinPerspective = ({
    ...
    if (!title) return null; // Logic to hide if no title

    return (
        <motion.div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
