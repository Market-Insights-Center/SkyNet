...
async def get_major_futures_data() -> Dict[str, Dict[str, Any]]:
    ...
    # --- Part 1.5: Get Gold data (Robust Logic) ---
    try:
        gold_url = 'https://www.cnbc.com/quotes/@GC.1'
        gold_scraped = await _scrape_cnbc_quote(gold_url)
        gold_hist = await asyncio.to_thread(yf.download, tickers=['GC=F'], period="5d", progress=False)

        if not gold_hist.empty and 'Close' in gold_hist:
            # Handle DataFrame vs Series from yfinance
            close_data = gold_hist['Close']
            series = None
            if isinstance(close_data, pd.DataFrame):
                if 'GC=F' in close_data.columns:
                     series = close_data['GC=F'].dropna()
            elif isinstance(close_data, pd.Series):
                series = close_data.dropna()
            ...
            if series is not None and len(series) > 1:
                ...
                prev_close = series.iloc[-2]
                final_price = gold_scraped if gold_scraped is not None else yf_price
                change_pct = ((final_price - prev_close) / prev_close) * 100
                results['GC=F'] = {'live_price': final_price, 'change_pct': change_pct}
            elif gold_scraped is not None:
                 results['GC=F'] = {'live_price': gold_scraped}
...
async def handle_briefing_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    spy_data, vix_data = spy_vix_data.get('SPY', {}), spy_vix_data.get('^VIX', {})
    risk_data_full = get_result('risk')
    risk_results = risk_data_full[0] if isinstance(risk_data_full, (list, tuple)) and len(risk_data_full) > 0 else {}
    yield_results = get_result('yields')
    ...
    watchlist_data = get_result('watchlist')
    
    top_3_stocks = breakout_results.get('current_breakout_stocks', [])[:3]
    top_3_breakout_tickers = [item.get('Ticker') for item in top_3_stocks]
    top_3_perf_data = await get_multi_period_change(top_3_breakout_tickers)

    # --- Final Report Data Construction ---
    ...
    return {
        ...
        "sp500_movers": sp_movers,
        "breakouts": {
            "stocks": top_3_stocks,
            "performance": top_3_perf_data
        },
        "economic_data": econ_results,
        "watchlist_movers": watchlist_data,
        "timestamp": datetime.now(EST_TIMEZONE).isoformat()
    }