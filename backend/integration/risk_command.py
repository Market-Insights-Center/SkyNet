...
async def perform_risk_calculations_singularity(is_eod_save: bool = False, is_called_by_ai: bool = False):
    ...
    results_summary = {
        "general_score": general if general is not None else 0,
        "large_cap_score": large_cap if large_cap is not None else 0,
        "ema_score": ema if ema is not None else 0,
        "combined_score": combined if combined is not None else 0,
        "market_invest_score": capped_mis_signal if capped_mis_signal is not None else 0,
        "market_score_percentile": market_percentile if market_percentile is not None else 50,
        "market_iv": market_iv if market_iv is not None else 0,
        "market_ivr": market_ivr if market_ivr is not None else 0,
        "recession_chance_ema": likelihood_ema_val if likelihood_ema_val is not None else 0,
        "recession_chance_vix": likelihood_vix_val if likelihood_vix_val is not None else 0,
    }
...
async def handle_risk_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    # We pass is_eod_save=True to force the calculation function to prepare CSV data if needed,
    # but we handle the actual saving here.
    results, data_to_save = await perform_risk_calculations_singularity(is_eod_save=True, is_called_by_ai=True)
