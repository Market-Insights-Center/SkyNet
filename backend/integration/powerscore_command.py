...
# GEMINI_API_LOCK imported from sentiment_command to share rate limiting
...
async def get_yf_data_singularity(tickers: List[str], period: str = "10y", interval: str = "1d", is_called_by_ai: bool = False) -> pd.DataFrame:
    ...
    data = await get_yf_download_robustly(
        ...
        auto_adjust=True, # Set to True as per FutureWarning
...
async def get_market_invest_score_for_powerscore() -> Optional[float]:
    ...
    try:
        ...
        # Clip the result to the 0-100 range and return as float
        if isinstance(market_invest_score, (pd.Series, np.ndarray, list)):
             market_invest_score = market_invest_score.iloc[0] if hasattr(market_invest_score, 'iloc') else market_invest_score[0]
...
def calculate_technical_indicators(data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        rs.replace([np.inf, -np.inf], np.nan, inplace=True) # Replace inf with NaN first
        rs = rs.ffill() # Forward fill NaNs if needed
        rs = rs.fillna(0) # Fill remaining NaNs at the start with 0
        data['RSI'] = 100 - (100 / (1 + rs))
        data['RSI'] = data['RSI'].fillna(50) # Fill any remaining NaNs with neutral 50
        ...
        # Fill MACD startup NaNs to avoid dropping data
        ...
        data['SMA_Diff'] = data['SMA_Diff'].fillna(0) # Fill NaNs resulting from sma200 being NaN
        ...
        data['Volatility'] = data['Volatility'].ffill() # Fill early NaNs
        data['Volatility'] = data['Volatility'].fillna(0) # Fill any remaining NaNs
...
async def handle_mlforecast_command_internal(ai_params: dict, is_called_by_ai: bool = True):
    ...
    if data.empty or len(data) < 100: # Reduced from 252 to 100 to allow newer stocks/IPOs
    ...
    for period_name, horizon in forecast_horizons_internal.items():
        try:
            ...
            cols_with_nan = df[features + ['Pct_Change']].isna().any()
            if cols_with_nan.any():
                print(f"   -> [DEBUG MLForecast] Dropping rows due to NaNs in: {cols_with_nan[cols_with_nan].index.tolist()}")

            df.dropna(subset=features + ['Pct_Change'], inplace=True)

            # Check if enough data remains after dropping NaNs and shifts
            if len(df) < 30: # Reduced from 50 to 30 for training
...
async def get_powerscore_explanation(ticker: str, component_scores: dict, model_to_use: Any, lock_to_use: asyncio.Lock) -> str:
    ...
    try:
        ...
        async with lock_to_use:
            ...
            for attempt in range(max_retries):
                try:
                    print(f"   -> [DEBUG PowerScore AI] Generating summary (Attempt {attempt+1}). Prompt length: {len(prompt)}")
                    response = await asyncio.to_thread(model_to_use.generate_content, prompt)
                    print(f"   -> [DEBUG PowerScore AI] Response received. Parts: {len(response.parts) if hasattr(response, 'parts') else 'N/A'}")
                    break
                except Exception as e:
                     print(f"   -> [DEBUG PowerScore AI] Attempt {attempt+1} failed: {e}")
                     if "429" in str(e) or "ResourceExhausted" in str(e):
                        if attempt < max_retries - 1:
                             # ... existing backoff logic ...
                             delay = initial_delay * (2 ** attempt) + random.uniform(0, 1)
                             print(f"Warning: PowerScore AI Rate Limit. Retrying in {delay:.2f}s...")
                             await asyncio.sleep(delay)
                        else: raise e
                     else: raise e
...
async def handle_powerscore_command(
    ...
    **kwargs # Add **kwargs to accept unexpected arguments
    ...
    # --- Gather Raw Data Asynchronously (with retries) ---
    tasks = {
        "R": with_retry(get_market_invest_score_for_powerscore, retries=2),
        "ABB_ABC": with_retry(get_single_stock_beta_corr, ticker, backtest_period, retries=2),
        "AA": with_retry(calculate_volatility_metrics, ticker, backtest_period, retries=2),
        "F": with_retry(handle_fundamentals_command_internal, ai_params={'ticker': ticker}, retries=2),
        "Q": with_retry(calculate_ema_invest, ticker, sensitivity, is_called_by_ai=True, retries=2),
        "S": with_retry(
        ...
        "M": with_retry(handle_mlforecast_command_internal, ai_params={'ticker': ticker}, retries=2)
    }
    ...
    elif isinstance(sentiment_result, dict) and sentiment_result.get("status") == "error":
         component_errors.append(f"S (Sentiment: {sentiment_result.get('message')})")
         raw['S'] = None
    else: raw['S'] = None
    ...
    if is_called_by_ai:
        ...
        return {
            ...
            "ai_explanation": await get_powerscore_explanation(ticker, prime, gemini_model, GEMINI_API_LOCK) if gemini_model else "AI Analysis Unavailable",
