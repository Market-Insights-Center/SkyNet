...
async def _run_convergence_matrix(prometheus_instance: Prometheus, run_id: int, run_name: str, universe_list: List[str], condition_list: List[str], strategy_list: List[str], db_path: str):
    """
    Executes the optimization loops for the convergence run.
    Iterates through every combination of Universe, Market Condition, and Strategy.
    """
    print(f"--- Starting Convergence Run {run_id}: '{run_name}' ---")
    ...
    for uni in universe_list:
        for cond in condition_list:
            ...
            start_date = None
            end_date = None
            period_arg = cond # Default: pass the name (e.g. "1y")
            
            if cond == "2022_Bear":
                start_date = "2022-01-01"; end_date = "2022-12-31"; period_arg = None
            elif cond == "2021_Bull":
                start_date = "2021-01-01"; end_date = "2021-12-31"; period_arg = None
            elif cond == "2020_Crash":
                start_date = "2020-02-01"; end_date = "2020-12-31"; period_arg = None
            elif cond == "Current_1Y":
                period_arg = "1y"
            
            for strat in strategy_list:
                print(f"\n>> Testing: {uni} | {cond} | {strat}")
                
                try:
                    # FIX: Unpack 3 values (params, return, metrics).
                    # This fixes the "too many values to unpack (expected 2)" error.
                    best_params, best_return, metrics = await prometheus_instance.run_parameter_optimization(
                    ...
                    if best_params:
                        # Save result using the helper function
                        await _save_convergence_result(
                            db_path=db_path,
                            run_id=run_id,
                            universe=uni,
                            condition=cond,
                    ...
                    print(f"❌ Error optimizing {strat} on {cond}: {e}")
                    # traceback.print_exc()

    print(f"\n--- [Convergence Run {run_id}] Finished All Permutations ---")
    
    # Generate summary using the updated function
    await _generate_convergence_summary(run_id, prometheus_instance, db_path)


async def _save_convergence_result(db_path: str, run_id, universe, condition, strategy, best_params, return_pct, metrics):
    """
    Helper function to save a single optimization result to the database.
    """
    try:
        async with aiosqlite.connect(db_path) as db:
            await db.execute("""
                INSERT INTO convergence_results 
                (run_id, universe, market_condition, strategy_name, best_params_json, 
                 total_return_pct, best_sharpe_ratio, trade_count, buy_hold_return_pct)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                run_id, 
                universe, 
                ...
                float(metrics.get("buy_hold_return_pct", 0.0))
            ))
            await db.commit()
    except Exception as e:
        print(f"❌ Error saving convergence result to DB: {e}")


async def _generate_convergence_summary(run_id: int, prometheus_instance: Prometheus, db_path: str):
    """
    Generates the summary table and AI report.
    """
    ...
    try:
        async with aiosqlite.connect(db_path) as db:
            ...
            async with db.execute("""
                SELECT strategy_name, market_condition, total_return_pct, buy_hold_return_pct, 
                       best_sharpe_ratio, trade_count, best_params_json 
        ...
        print(f"❌ Error fetching results for summary: {e}")
        return
    ...
    # Prepare Data for Table & AI
    table_data = []
    for r in results:
        # Calculate Alpha
        ret = r['total_return_pct']
        bh = r['buy_hold_return_pct']
        # Handle cases where bh might be None
        bh_val = bh if bh is not None else 0.0
        alpha = ret - bh_val
        
        trades = r['trade_count']
        freq = f"{trades/12:.1f}/mo" if trades > 12 else f"{trades}/yr"
        ...
        table_data.append([
            ...
            f"{alpha:.2f}%",
            freq,
            f"{r['best_sharpe_ratio']:.2f}",
            r['best_params_json']
        ])

    headers = ["Strategy", "Condition", "Return", "Buy&Hold", "Alpha", "Freq", "Sharpe", "Params"]
    ...
    try:
        prompt = f"""
        Act as a financial analyst. Analyze these trading strategy backtest results.
        Identify the winner based on Return and Sharpe Ratio.
        Compare it to Buy & Hold.
        ...
        """
        # Ensure gemini_model exists (OllamaWrapper)
        if prometheus_instance.gemini_model:
            response = await prometheus_instance.gemini_model.generate_content_async(prompt)
            print("\n--- 🤖 Convergence AI Summary Report 🤖 ---")
            print(response.text)
            print("------------------------------------------")
        else:
            print("   -> AI Model not available for summary.")
...
async def _handle_kronos_convergence(parts: List[str], prometheus_instance: Prometheus):
    """
    Handles the 'convergence' command parsing and execution.
    """
    if len(parts) < 2:
        print("Usage: convergence <run_name> --universes=... --conditions=... --strategies=...")
        return
    ...
    # Defaults
    universes = ["SPY"]
    conditions = ["Current_1Y"]
    strategies = ["ma_crossover"]
    
    # Parse flags
    for part in parts[2:]:
        if part.startswith("--universes="):
            universes = part.split("=")[1].split(",")
        elif part.startswith("--conditions="):
            conditions = part.split("=")[1].split(",")
        elif part.startswith("--strategies="):
            strategies = part.split("=")[1].split(",")

    print(f"   -> Setup Convergence Run '{run_name}'")
    print(f"      Universes: {universes}")
    print(f"      Conditions: {conditions}")
    print(f"      Strategies: {strategies}")

    # Generate Run ID (simple timestamp integer)
    run_id = int(datetime.now().timestamp())
    
    # Execute Matrix
    # FIX: Argument names now match _run_convergence_matrix definition exactly
    await _run_convergence_matrix(
        prometheus_instance=prometheus_instance,
        run_id=run_id,
        run_name=run_name,
        universe_list=universes,   # Changed from 'universes' to 'universe_list'
        condition_list=conditions, # Changed from 'conditions' to 'condition_list'
        strategy_list=strategies,  # Changed from 'strategies' to 'strategy_list'
...
def _load_kronos_config() -> Dict[str, Any]:
    ...
    Updated to include self_drive_interval_hours.
    """
    config = {
        "correlation_interval_hours": DEFAULT_CORR_INTERVAL_HOURS,
        "workflow_analysis_chance": DEFAULT_WORKFLOW_CHANCE,
        "self_drive_interval_hours": 24 # Default to 24h if not set
    }
    try:
        if os.path.exists(PROMETHEUS_STATE_FILE):
            with open(PROMETHEUS_STATE_FILE, 'r') as f:
                state = json.load(f)
                # Load keys if they exist in the file, otherwise keep defaults
                if "correlation_interval_hours" in state:
                    config["correlation_interval_hours"] = state["correlation_interval_hours"]
                if "workflow_analysis_chance" in state:
                    config["workflow_analysis_chance"] = state["workflow_analysis_chance"]
                if "self_drive_interval_hours" in state:
                    config["self_drive_interval_hours"] = state["self_drive_interval_hours"]
...
async def _handle_kronos_test(parts: List[str], prometheus_instance: Prometheus):
    ...
    try:
        ...
        original_target_path_check = os.path.join(os.path.dirname(__file__), filename_to_improve)
        ...
        print("\n--- Confirmation ---")
        original_target_path = os.path.join(os.path.dirname(__file__), filename_to_improve)
...
async def _handle_kronos_config(parts: List[str], prometheus_instance: Prometheus):
    """
    View or set configuration parameters for Kronos automation.
    """
    ...
    # ALLOWED KEYS - ADD 'self_drive_interval_hours' HERE
    ALLOWED_KEYS = [
        "correlation_interval_hours", 
        "workflow_analysis_chance",
        "self_drive_interval_hours" 
    ]
    ...
    if key not in ALLOWED_KEYS:
        print(f"❌ Error: Unknown or read-only config key '{key}'.")
        print(f"   Available keys: {', '.join(ALLOWED_KEYS)}")
        return

    if len(parts) < 3:
        print(f"Current value for '{key}': {config.get(key, 'Not Set')}")
    else:
        try:
            value = float(parts[2])
            config[key] = value
            ...
            # If changing the self-drive interval, notify user restart might be needed for immediate effect
            if key == "self_drive_interval_hours":
                print("   (Note: The new interval will take effect after the NEXT scheduled check or upon restart.)")
...
async def kronos_self_drive_worker(prometheus_instance: Prometheus):
    """
    [Quiet Version] Background worker.
    Runs silently. Only prints on startup, error, or successful update.
    """
    print("🧠 Kronos Self-Drive Module: Online (Silent Mode).")
    
    while True:
        try:
            # 1. Run the Analysis (Silently)
            if prometheus_instance.is_active:
                # We removed the "Initiating..." print here
                result = await prometheus_instance.run_autonomous_optimization("SPY")
                
                # We only print if something actually happened
                if result and result.get("status") == "success":
                    pass # The Prometheus method now handles the success print
            
            # 2. Check Config
            config = _load_kronos_config()
            interval_hours = config.get("self_drive_interval_hours", 24)
            
            # Removed the "Sleeping..." print to keep terminal clean
            
            # 3. Sleep
            await asyncio.sleep(interval_hours * 3600) 
            
        except asyncio.CancelledError:
            print("Kronos Self-Drive Module: Offline.")
            break
        except Exception as e:
            # We DO print errors so you know if it breaks
            print(f"❌ Kronos Self-Drive Error: {e}")
            await asyncio.sleep(3600)

# --- ADD THIS FUNCTION to handle manual command triggering ---
async def _handle_kronos_self_drive(parts: List[str], prometheus_instance: Prometheus):
    """
    [New] Manually triggers the self-drive analysis command.
    Usage: selfdrive
    """
    print("Manual trigger: Kronos Self-Drive Analysis...")
    
    if not prometheus_instance.is_active:
        print("❌ Error: Prometheus is INACTIVE. Activate it first.")
        return

    result = await prometheus_instance.run_autonomous_optimization("SPY")
    
    if result:
        print("\n--- Manual Self-Drive Report ---")
        print(result.get("report", "No report generated."))
        if result.get("status") == "success":
            print("\n✅ Live strategy file updated successfully.")
    else:
        print("❌ Optimization failed or returned no result.")
...
async def handle_kronos_command(args: List[str], prometheus_instance: Prometheus):
    ...
    Updated to include the 'selfdrive' autonomous command.
    """
    ...
    while True:
        try:
            ...
            if cmd == 'exit':
                ...
                print("  selfdrive            : [Phase 6] Trigger the autonomous optimization loop manually.")
                print("                         (Detects regime -> Recalls history -> Optimizes -> Saves live strategy)")
                print("  discover <name> --condition=... : [Phase 5] Run workflow optimization.")
                print("  convergence <name> ... : [Phase 1-4] Run meta-optimization for strategy parameters.")
                print("  optimize <strat> ... : Run GA parameter optimization for a single strategy.")
                print("  test <file> ...      : Run the 'Hypothesize -> Generate -> Test' loop.")
                print("  schedule <add|list>... : Manage scheduled tasks.")
                print("  analyze logs ...     : Analyze the Prometheus command log database.")
                print("  cache <list|clear>   : View or clear data caches.")
                ...
                # This calls the manual trigger function we added earlier
                await _handle_kronos_self_drive(parts, prometheus_instance)
