...
async def handle_invest_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False, return_structured_data: bool = False):
    ...
    if ai_params:
        print(f"DEBUG: handle_invest_command called with ai_params: {ai_params}")
        try:
            ema_sens = int(ai_params.get("ema_sensitivity", 2))
            ...
            portfolio_data_config_invest['amplification'] = str(float(ai_params.get("amplification", 1.0)))
            ...
            for i, sub_p in enumerate(sub_portfolios, 1):
                tickers = sub_p.get("tickers")
                weight = sub_p.get("weight")
        ...
        print(f"DEBUG: Calling process_custom_portfolio with config: {portfolio_data_config_invest}")
        ...
        tailored_list_str, combined_data, final_cash, tailored_structured_data = await process_custom_portfolio(
            ...
            tailor_portfolio_requested=tailor_run,
            frac_shares_singularity=frac_s_run, 
            total_value_singularity=total_val_run,
            is_custom_command_simplified_output=tailor_run, 
            is_called_by_ai=True
        )
        
        print(f"DEBUG: process_custom_portfolio returned combined_data len: {len(combined_data)}, tailored_structured_data len: {len(tailored_structured_data)}")
        ...
        try:
            ...
            if tailor_str_cli == 'yes':
                ...
                portfolio_data_config_invest['frac_shares'] = str(frac_s_str_cli).lower()
