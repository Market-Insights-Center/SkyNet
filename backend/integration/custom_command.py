# backend/integration/custom_command.py
...
# Local safe_score override to handle commas
...
async def handle_custom_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    try:
        if ai_params: 
            ...
            if action == "run_existing_portfolio":
                    ...
                    try:
                        with open(PORTFOLIO_DB_FILE, 'r', encoding='utf-8', newline='') as f_db:
                            ...
                            for row in reader:
                                ...
                                row_user = row.get('user_id', '').strip() if row.get('user_id') else None
                                target_user = user_id.strip() if user_id else None
                                
                                is_code_match = row_code == portfolio_code_input.strip().lower()
                                is_owner_match = (row_user == target_user) or (not row_user)

                                if is_code_match and is_owner_match:
                ...
                if not portfolio_config_from_db:
                    ...
                    if sub_portfolios_list and "ema_sensitivity" in ai_params:
                        # CREATE IT
                        new_config = {
                        ...
                        return {
                            "status": "not_found", 
                            "message": f"Portfolio '{portfolio_code_input}' not found or permission denied.",
                ...
                if tailor_run_ai:
                    try:
                        val_str = ai_params.get("total_value", 0)
                        total_value_ai_float = safe_score(val_str)
                        if total_value_ai_float <= 0: return {"status": "error", "message": "'total_value' must be positive."}
                    except ValueError: 
                        return {"status": "error", "message": "Invalid 'total_value'."}
                ...
                # Map the internal data keys to the keys expected by Results.jsx ('value', 'price', 'shares')
                ...
                for item in tailored_data:
                    formatted_table_data.append({
                        ...
                        "value": item.get('actual_money_allocation'), # Critical for sorting/charts in Results.jsx
                ...
                return {
                    ...
                    "table": formatted_table_data, # Results.jsx expects 'table'
                    "raw_result": {
                        "final_cash": final_cash,
                        "portfolio_code": portfolio_code_input
                    }
        ...
        traceback.print_exc()
        return {"status": "error", "message": f"Critical Error in Custom Command: {str(e)}"}