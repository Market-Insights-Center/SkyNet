...
async def main():
    ...
    try:
        async with websockets.serve(handler, "0.0.0.0", WS_PORT):
            print("Server Listening. Please refresh the Orion web page now.")
            await asyncio.get_running_loop().create_future()  # run forever
    except OSError as e:
        print(f"ERROR: Could not bind to port {WS_PORT}. This means Orion Backend is ALREADY RUNNING.")
        print("Please terminate the existing 'orion_v2.py' process and run this test again.")
